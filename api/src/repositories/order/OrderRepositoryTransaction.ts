...
export default class OrderRepositoryTransaction implements OrderRepository {
  ...
  async createOrder(order: Order): Promise<Order> {
    ...
    await conn.execute(
      ...
      [order.id, order.ref_user, order.total, order.state, Object.values(Type).indexOf(order.type), Object.values(Payment).indexOf(order.payment), new Date()]);

    for (const item of order.items ?? []) {
      item.ref_order = order.id;

      await conn.execute(
        'INSERT orders_items SET ref_order = ?, ref_item = ?',
        [order.id, item.ref_item]);

      await conn.execute('UPDATE items SET state = ? WHERE id = ?', [State.DELIVERED, item.ref_item]);
    }
